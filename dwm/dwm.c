...
clientmessage(XEvent *e)
{
	...
	/* _NET_CURRENT_DESKTOP is repurposed as a monitor selector for the
	 * quickshell bar (via xdotool set_desktop); it arrives on the root
	 * window, so it must be handled before the client guard below */
	if (cme->message_type == netatom[NetCurrentDesktop]) {
		for (m = mons; m; m = m->next)
			if (m->num == cme->data.l[0])
				break;
		if (m && m != selmon) {
...
/* json-escape s into d (capacity n) */
void
jsonescape(const char *s, char *d, size_t n)
{
	size_t i = 0;
...
manage(Window w, XWindowAttributes *wa)
{
	...
	Window trans = None;
	XWindowChanges wc;
	Client dockcheck = { .win = w };

	/* dock-type windows (external bars, launchers) position themselves;
	 * map them unmanaged instead of tiling them */
	if (getatomprop(&dockcheck, netatom[NetWMWindowType]) == netatom[NetWMWindowTypeDock]) {
		XMapRaised(dpy, w);
		return;
	}
...
tagmon(const Arg *arg)
{
	...
	/* follow the window: keep it focused on its new monitor so it can be
	 * bounced back with the same keybind, no mouse travel needed */
	focus(c);
}
